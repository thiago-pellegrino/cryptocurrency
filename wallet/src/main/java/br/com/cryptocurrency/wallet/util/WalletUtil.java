package br.com.cryptocurrency.wallet.util;
...
public class WalletUtil {
	...
	public static boolean validateBalance(Blockchain blockchain, Wallet wallet) {
		log.info("### Etapa Transação: verificando o tipo de transacao");
		if (wallet.getTransaction().getTransactionType() == EnumTransactionType.EXCHANGE) {
			log.info("### Etapa Transação: EXCHANGE");
			return true;
		}
		BigDecimal totalAmount = new BigDecimal(0);
		log.info("### Etapa Transação: calculando o balance do sender varrendo toda a blockchain");
		...
		log.info("### Etapa Transação: verificando se o valor da transacao é maior que o balance");
